...
export default function LobbyPage() {
  ...
  const joinRoom = () => {
    ...
    socket.once("player_joined", (player) => {
      navigate(`/player?roomId=${code}&playerId=${player.id}`)
    })
      ...
      <div className="w-full max-w-md">
        ...
        {mode === "create" && (
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white rounded-lg p-4">
              ...
              <input
                type="range" min={5} max={15} value={winScore}
            ...
            <div className="bg-white rounded-lg p-4">
              <label className="flex items-center gap-2 text-black">
                <input type="checkbox" checked={sequelConfusion}
        ...
        {mode === "join" && (
          <div className="flex flex-col gap-4">
            ...
            <input
              type="text"
              placeholder="Room code (e.g. K7BX2M)"
              value={roomCode}
              onChange={e => { setRoomCode(e.target.value.toUpperCase()); setError("") }}
              className="border p-3 rounded-lg text-black bg-white text-lg font-mono tracking-widest uppercase"
              maxLength={6}
            />
